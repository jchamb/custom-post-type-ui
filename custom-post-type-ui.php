...
function cpt_create_custom_post_types() {
	...
	if ( is_array( $cpt_post_types ) ) {
		foreach ($cpt_post_types as $cpt_post_type) {
            ...
            $cpt_menu_icon          = ( !empty( $cpt_post_type["menu_icon"] ) ) ? (strpos('dashicons-', $cpt_post_type["menu_icon"]) === 0) ? $cpt_post_type["menu_icon"] : esc_url( $cpt_post_type["menu_icon"] ) : null; //must be null
